...
var OpenFoodFacts = (function() {
  ...
  function detectCategory(product) {
    ...
    var scores = {
      dairy: 0, meat: 0, produce: 0, beverages: 0,
      frozen: 0, bakery: 0, sweets: 0, pantry: 0
    };

    // ===== PUNTEGGIO LATTICINI (solo keyword FORTI del nome) =====
    var dairyNameKw = [
      'latte', 'formaggio', 'yogurt', 'yoghurt', 'latticino', 'latticini',
      'mozzarella', 'parmigiano', 'ricotta', 'burro', 'panna', 'pannette',
      'pannarello', 'latte condensato', 'latte in polvere', 'latte fresco',
      'latte uht', 'latte intero', 'latte scremato', 'latte parzialmente scremato',
      ...
      'yogurt bianco', 'yogurt alla frutta', 'kefir', 'gelato', 'sorbetto',
      'semifreddo', 'crema pasticcera', 'crema chantilly', 'crema inglese',
      'philadelphia', 'spalmabile', 'whipped cream', 'sour cream',
      'double cream', 'single cream', 'clotted cream', 'crème fraîche',
      'crème fraiche', 'milk', 'cheese', 'dairy', 'cream', 'butter',
      'margarine', 'ghee', 'quark', 'cheddar', 'swiss cheese',
      'cottage cheese', 'cream cheese', 'whipping cream', 'heavy cream',
      'half and half', 'evaporated milk', 'condensed milk', 'powdered milk',
      'dry milk', 'skim milk', 'whole milk', 'low fat milk',
      'soy milk', 'almond milk', 'oat milk', 'rice milk', 'coconut milk',
      'plant milk', 'plant-based milk', 'non-dairy milk',
      'frozen yogurt', 'frozen custard', 'whey', 'casein', 'lactose',
      'probiotic', 'probiotics', 'fermented milk',
      'fromage', 'lait', 'crème', 'beurre', 'yaourt',
      'queso', 'leche', 'crema', 'mantequilla', 'yogur',
      'käse', 'milch', 'sahne', 'joghurt'
    ];
    for (var dn = 0; dn < dairyNameKw.length; dn++) {
      if (name.indexOf(dairyNameKw[dn]) !== -1) scores.dairy += 10;
    }

    // ===== PUNTEGGIO CARNE E PESCE =====
    var meatNameKw = [
      ...
      'salame', 'mortadella', 'bologna', 'cotechino', 'zampone',
      'pancetta', 'guanciale', 'lardo', 'speck', 'jamón', 'jamon',
      'chorizo', 'soppressa', 'nduja', 'filetto', 'costata',
      'costoletta', 'spiedino', 'spiedini', 'hot dog', 'hotdog',
      'frankfurter', 'frankfurters', 'cotoletta', 'scaloppina',
      'brasato', 'stufato', 'ragù', 'sugo di carne', 'brodo di carne',
      'brodo di pollo', 'brodo di manzo', 'dado', 'dadi',
      'brodo granulare', 'gambero', 'gamberi', 'gamberetto',
      'gamberetti', 'aragosta', 'astice', 'granchio', 'polpo',
      'calamaro', 'calamari', 'seppia', 'seppie', 'cozza', 'cozze',
      ...
      'pesce azzurro', 'pesce fresco', 'pesce surgelato', 'surimi',
      'fish stick', 'fish sticks', 'fish finger', 'fish fingers',
      'bastoncini di pesce', 'burger di pesce', 'burger di pollo',
      'nugget', 'nuggets', 'crocchetta', 'crocchette', 'polpetta',
      'polpette', 'polpettone', 'salsicce', 'salsicciotto',
      'salsicciotti', 'soppressata',
      'meat', 'beef', 'pork', 'lamb', 'veal', 'turkey', 'duck',
      'chicken', 'fish', 'seafood', 'steak', 'fillet', 'bacon',
      'ham', 'sausage', 'salami', 'pepperoni', 'meatball', 'meatballs',
      'patty', 'patties', 'ground beef', 'minced meat', 'mince',
      'meatloaf', 'roast beef', 'roast pork', 'roast chicken',
      'roast lamb', 'ribs', 'rib', 'chop', 'chops', 'tenderloin',
      'sirloin', 'rump', 'flank', 'brisket', 'shank', 'oxtail',
      'tripe', 'liver', 'kidney', 'heart', 'shrimp', 'prawn',
      'prawns', 'lobster', 'crab', 'scallop', 'scallops', 'mussel',
      'mussels', 'clam', 'clams', 'oyster', 'oysters', 'octopus',
      'squid', 'calamari', 'anchovy', 'anchovies', 'sardine',
      'sardines', 'mackerel', 'cod', 'haddock', 'hake', 'plaice',
      'sole', 'turbot', 'halibut', 'tuna', 'tuna fish', 'swordfish',
      'marlin', 'mahi mahi', 'snapper', 'grouper', 'barramundi',
      'trout', 'salmon', 'smoked salmon', 'gravlax', 'lox', 'caviar',
      'roe', 'fish cake', 'fish cakes', 'crab cake', 'crab cakes',
      ...
      'fried chicken', 'bbq chicken', 'grilled chicken', 'beef jerky',
      'biltong', 'dried meat', 'cured meat', 'charcuterie',
      'deli meat', 'lunch meat', 'cold cuts', 'pâté', 'pate',
    ...
    for (var mn = 0; mn < meatNameKw.length; mn++) {
      if (name.indexOf(meatNameKw[mn]) !== -1) scores.meat += 10;
    }

    // ===== PUNTEGGIO VERDURA E FRUTTA =====
    var produceNameKw = [
      ...
      'cranberry', 'cranberries', 'goji', 'acai', 'baobab',
      'camu camu', 'melone', 'meloni', 'anguria', 'cocomero',
      'cocomeri', 'papaia', 'guava', 'feijoa', 'jujube',
      'carambola', 'pitaya', 'dragon fruit', 'jackfruit',
      'breadfruit', 'avocado', 'avocado', 'oliva', 'olive',
      'pomodoro ciliegino', 'pomodorini', 'datterino', 'datterini',
      'san marzano', 'cuore di bue', 'costoluto', 'pachino',
      'aneto', 'coriandolo', 'alghe', 'alga', 'nori', 'wakame',
      'kombu', 'dulse', 'spirulina', 'chlorella', 'kelp',
      'sea vegetable', 'legume', 'legumes', 'pulse', 'pulses',
      ...
      'légume', 'légumes', 'gemüse', 'obst', 'groente', 'vruchten'
    ];
    for (var pn = 0; pn < produceNameKw.length; pn++) {
      if (name.indexOf(produceNameKw[pn]) !== -1) scores.produce += 10;
    }
    ...
    var beverageNameKw = [
      ...
      'camomilla', 'limone', 'frutti di bosco',
      ...
      'protein shake', 'coffee', 'coffees', 'espresso', 'cappuccino',
      'latte', 'americano', 'macchiato', 'mocha', 'flat white', 'cortado',
      ...
      'amber ale', 'brown ale', 'bitter', 'barley wine',
      ...
      'drank', 'dranken', 'wijn', 'wijnen', 'sap', 'sappen',
      'bier', 'bieren', 'likeur', 'likeuren'
    ];
    for (var bn = 0; bn < beverageNameKw.length; bn++) {
      if (name.indexOf(beverageNameKw[bn]) !== -1) scores.beverages += 10;
    }
    ...
    var frozenNameKw = [
      ...
      'sorbet', 'sorbets', 'tiefgefroren', 'tiefkühl',
      'tiefgekühlt', 'tiefkühlkost', 'tiefkühlprodukt',
      'diepvries', 'diepgevroren', 'ingevroren', 'vrieskast'
    ];
    for (var fn = 0; fn < frozenNameKw.length; fn++) {
      if (name.indexOf(frozenNameKw[fn]) !== -1) scores.frozen += 10;
    }

    // ===== PUNTEGGIO PANETTERIA =====
    var bakeryNameKw = [
      ...
      'pizza', 'pizze', 'pizzetta', 'pizzette',
      'piadina', 'piadine', 'tigella', 'tigelle', 'crescentina', 'crescentine',
      'gnocco', 'gnocchi', 'gnocco fritto',
      'pagnotta', 'pagnotte', 'filone', 'filoni',
      ...
      'danish', 'danishes', 'pastry', 'pastries',
      'biscuit', 'biscuits', 'cookie', 'cookies', 'brownie', 'brownies',
      'blondie', 'blondies', 'cake', 'cakes', 'cupcake', 'cupcakes',
      'layer cake', 'pound cake', 'sponge cake', 'angel food cake',
      'chiffon cake', 'cheesecake', 'cheesecakes', 'tiramisu', 'tiramisù',
      'pie', 'pies', 'tart', 'tarts', 'quiche', 'quiches',
      'flan', 'flans', 'custard tart', 'fruit tart',
      'calzone', 'calzones', 'stromboli',
      'brot', 'brötchen', 'pain', 'pains', 'viennoiserie', 'pâtisserie',
      'pan', 'panes', 'bollería', 'bollo', 'bollos',
      'brood', 'broden', 'broodje', 'broodjes', 'stokbrood'
    ];
    for (var ban = 0; ban < bakeryNameKw.length; ban++) {
      if (name.indexOf(bakeryNameKw[ban]) !== -1) scores.bakery += 10;
    }
    ...
    var sweetsNameKw = [
      ...
      'after eight', 'raffaello', 'ferrero collection', 'mon chéri',
      ...
      'caramel', 'caramels', 'butterscotch',
      ' Turkish delight', 'lokum', 'halva', 'halvah',
      'turnover', 'turnovers', 'strudel', 'strudels',
      'baklava', 'baklavas', 'mochi', 'mochis', 'wagashi', 'wagashis',
      'confiserie', 'confiseries', 'chocolat', 'chocolats',
      'sucre', 'sucré', 'sucreries',
      'dulce', 'dulces', 'caramelo', 'caramelos',
      'süßigkeit', 'süßigkeiten', 'schokolade', 'schokoladen',
      'praline', 'pralinen', 'nougat',
      'snoep', 'snoepjes', 'snoepgoed', 'chocolade', 'chocolades',
      'likeur', 'likeuren'
    ];
    for (var sn = 0; sn < sweetsNameKw.length; sn++) {
      if (name.indexOf(sweetsNameKw[sn]) !== -1) scores.sweets += 10;
    }

    // Punteggio extra da categorie/labels/brand/ingredienti
    var allText = cats + ' ' + labels + ' ' + brand + ' ' + ingredients;
    if (allText.indexOf('dairy') !== -1) scores.dairy += 5;
    if (allText.indexOf('meat') !== -1) scores.meat += 5;
    if (allText.indexOf('fish') !== -1) scores.meat += 5;
    if (allText.indexOf('seafood') !== -1) scores.meat += 5;
    if (allText.indexOf('vegetable') !== -1) scores.produce += 5;
    if (allText.indexOf('fruit') !== -1) scores.produce += 5;
    if (allText.indexOf('beverage') !== -1) scores.beverages += 5;
    if (allText.indexOf('drink') !== -1) scores.beverages += 5;
    if (allText.indexOf('frozen') !== -1) scores.frozen += 5;
    if (allText.indexOf('bread') !== -1) scores.bakery += 5;
    if (allText.indexOf('bakery') !== -1) scores.bakery += 5;
    if (allText.indexOf('pastry') !== -1) scores.bakery += 3;
    if (allText.indexOf('sweet') !== -1) scores.sweets += 5;
    if (allText.indexOf('chocolate') !== -1) scores.sweets += 5;
    if (allText.indexOf('candy') !== -1) scores.sweets += 5;
    if (allText.indexOf('confectionery') !== -1) scores.sweets += 5;

    // Trova la categoria con il punteggio più alto
    var bestCategory = 'pantry';
    var bestScore = 0;
