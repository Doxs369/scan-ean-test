...
function deleteProduct() {
  ...
  // Salva il prodotto per il modal di conferma
  window.pendingActionProduct = p;
  window.pendingActionType = 'delete';

  // Mostra modal di conferma
  showActionConfirmModal(p.name, 'rimosso');
}
...
// ============================================================
// MODAL CONFERMA AZIONE PRODOTTO (Consuma/Elimina)
// ============================================================
var pendingActionProduct = null;
var pendingActionType = null;

/**
 * Mostra il modal di conferma azione
 */
function showActionConfirmModal(productName, actionText) {
  document.getElementById('action-confirm-product').textContent = '"' + productName + '"';
  document.getElementById('action-confirm-text').textContent = 
    'Il prodotto verra ' + actionText + '. Cosa vuoi fare?';
  document.getElementById('action-confirm-modal').classList.add('show');
}

/**
 * Chiude il modal di conferma azione
 */
function closeActionConfirmModal(e) {
  if (e && e.target !== e.currentTarget) return;
  document.getElementById('action-confirm-modal').classList.remove('show');
  pendingActionProduct = null;
  pendingActionType = null;
}

/**
 * Sposta il prodotto in lista della spesa
 */
function moveToShoppingList() {
  if (!pendingActionProduct) return;

  var reason = pendingActionType === 'consume' ? 'Consumato' : 'Rimosso';
  shoppingList.push({ 
    id: nextListId++, 
    name: pendingActionProduct.name, 
    checked: false, 
    reason: reason 
  });
  Storage.saveShoppingList(shoppingList);
  ...
  removeProductFromPantry(pendingActionProduct.id);
  ...
  showToast('&#128722; ' + pendingActionProduct.name + ' aggiunto alla lista spesa');
...
/**
 * Elimina solo dalla dispensa (non in lista spesa)
 */
function deleteOnlyFromPantry() {
  if (!pendingActionProduct) return;
  ...
  showToast('&#128465; ' + pendingActionProduct.name + ' eliminato dalla dispensa');
...
/**
 * Rimuove un prodotto dalla dispensa (helper)
 */
function removeProductFromPantry(productId) {
  Storage.deleteRecipes(productId);

  var newProducts = [];
  for (var j = 0; j < products.length; j++) {
    if (products[j].id !== productId) newProducts.push(products[j]);
  }
  products = newProducts;
  Storage.saveProducts(products);
  cleanupRecipes();
