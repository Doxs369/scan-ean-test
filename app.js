 ...
 * Scanner barcode + Open Food Facts + TheMealDB ricette anti-spreco
 */
...
var dailyRecipes = null;  // Ricette caricate oggi
...
function init() {
  ...
  // PULIZIA GIORNALIERA: scaduti → lista spesa, ricette vecchie
  runDailyCleanup();

  // Carica ricette del giorno (se esistono)
...
function runDailyCleanup() {
  ...
  if (lastCleanup === today) return; // Già fatto oggi

  console.log('Running daily cleanup...');

  // 1. Sposta prodotti scaduti nella lista spesa
  var expired = RecipesAPI.getExpiredProducts(products);
  for (var i = 0; i < expired.length; i++) {
    var p = expired[i];
    // Evita duplicati nella lista spesa
  ...
  // Rimuovi prodotti scaduti dalla dispensa
  if (expired.length > 0) {
    var newProducts = [];
    for (var k = 0; k < products.length; k++) {
      var isExpired = false;
      for (var m = 0; m < expired.length; m++) {
        if (products[k].id === expired[m].id) {
          isExpired = true;
          break;
        }
      }
  ...
  // 2. Pulisci ricette vecchie
  RecipesAPI.cleanupOldRecipes();

  // 3. Pulisci ricette orfane
  cleanupRecipes();

  // 4. Marca cleanup come fatto
...
function loadDailyRecipesFromAPI() {
  ...
  // Prendi il primo prodotto che scade oggi
    ...
    .then(function(recipes) {
      if (recipes.length > 0) {
        dailyRecipes = {
          ...
          recipes: recipes.slice(0, 5) // Max 5 ricette
        };
        RecipesAPI.saveDailyRecipes(dailyRecipes);
        renderDailyRecipes();
        showToast('&#9989; ' + recipes.length + ' ricette trovate!');
      } else {
        dailyRecipes = null;
        renderDailyRecipes();
...
function renderDailyRecipes() {
  ...
  if (!dailyRecipes || !dailyRecipes.recipes || dailyRecipes.recipes.length === 0) {
    var expiringToday = RecipesAPI.getExpiringToday(products);
    if (expiringToday.length > 0) {
      container.innerHTML =
        '<div class="recipe-card" style="cursor:pointer;" onclick="loadDailyRecipesFromAPI()">' +
          '<div class="recipe-header">' +
            ...
            '<div>' +
              '<div class="recipe-title">Trova ricette per oggi</div>' +
              '<div class="recipe-ingredients">Clicca per cercare ricette con "' + expiringToday[0].name + '"</div>' +
            '</div>' +
          '</div>' +
          '<span class="recipe-match">&#128269; Cerca su TheMealDB</span>' +
        '</div>';
    } else {
      container.innerHTML = '';
    }
      ...
      '<div class="recipe-card" style="cursor:pointer;" onclick="openMealDBRecipe(' + r.id + ')">' +
        '<div class="recipe-header">' +
          ...
          '<div style="min-width:0;">' +
            ...
            '<div class="recipe-ingredients">&#127860; Ricetta da TheMealDB</div>' +
          '</div>' +
        '</div>' +
        '<span class="recipe-match">&#128073; Tocca per i dettagli</span>' +
      '</div>';
  }
...
function handleCameraPhoto(event) {
  ...
  // Limita dimensione: se > 500KB, comprimi
  if (file.size > 500 * 1024) {
    compressImage(file, 500, function(compressedDataUrl) {
      cameraPhotoData = compressedDataUrl;
...
/**
 * Comprime immagine per risparmiare spazio localStorage
 */
function compressImage(file, maxKB, callback) {
  ...
  reader.onload = function(e) {
    ...
    img.onload = function() {
      ...
      // Scala a max 400px di larghezza
      var scale = Math.min(1, 400 / img.width);
      canvas.width = img.width * scale;
      canvas.height = img.height * scale;
      ctx.drawImage(img, 0, 0, canvas.width, canvas.height);

      // Comprimi con qualità decrescente fino a sotto maxKB
      var quality = 0.7;
      var dataUrl;
      do {
...
function consumeProduct() {
  ...
  // Rimuovi dalla dispensa (il prodotto è stato consumato)
  var newProducts = [];
  for (var j = 0; j < products.length; j++) {
    if (products[j].id !== selectedProductId) {
      newProducts.push(products[j]);
...
// ============================================================
// RICETTE (LEGACY - Gemini, mantenuto per compatibilità)
// ============================================================
function loadRecipesForProduct() {
  // Disabilitato: ora usiamo TheMealDB nella schermata principale
...
// ============================================================
// CAPTURE BARCODE MANUALE
// ============================================================
function captureBarcode() {
  // Scatta foto dal video e prova a leggere barcode
  var frameData = BarcodeScanner.scanFrame();
  if (frameData) {
    showToast('&#128247; Frame catturato, analisi...');
    // Se ZXing è caricato, prova a decodificare dal canvas
