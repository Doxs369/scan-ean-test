...
var currentSort = 'expiry'; // 'expiry' | 'name' | 'added' | 'category'
var nextId = 1;
var nextListId = 1;
...
// ============================================================
// RICETTE DEL GIORNO — SOLO PRODOTTO IN SCADENZA OGGI
// ============================================================
function loadDailyRecipesFromAPI() {
  ...
  // Se già abbiamo ricette per oggi per questo prodotto, non richiamare
  ...
  RecipesAPI.searchByIngredient(product.name)
    .then(function(recipes) {
      if (recipes.length > 0) {
        ...
        // Nessuna ricetta trovata — mostra messaggio specifico
        dailyRecipes = {
          productName: product.name,
          productId: product.id,
          recipes: [],
...
function renderDailyRecipes() {
  ...
  // Header che mostra IL PRODOTTO specifico in scadenza
  var headerHtml =
  ...
  // Se non abbiamo ancora caricato le ricette
  ...
  // Ricette caricate ma nessun risultato
  ...
  // Mostra le ricette trovate per QUESTO prodotto
  var html = headerHtml;
...
/**
 * Ordina i prodotti in base al criterio selezionato
 */
function sortProducts(list) {
  ...
  sorted.sort(function(a, b) {
    if (currentSort === 'expiry') {
      // Prima scaduti, poi in scadenza, poi per data
  ...
  if (currentFilter !== 'all') {
    if (currentFilter === 'expiring') {
      ...
      for (var e = 0; e < filtered.length; e++) {
        if (getDaysUntilExpiry(filtered[e].expiryDate) < 0) {
          tempE.push(filtered[e]);
        }
      }
      filtered = tempE;
    } else {
...
function updateStats() {
  ...
  // Aggiorna anche i click handler delle card stats
  updateStatCardsClickHandler();
}

/**
 * Aggiorna i click handler delle card statistiche
 */
function updateStatCardsClickHandler() {
  var statCards = document.querySelectorAll('.stat-card');
  if (statCards.length >= 3) {
    // Prodotti totali → resetta filtri
    ...
    // In scadenza → filtra prodotti con 0-3 giorni
    statCards[1].onclick = function() {
      currentFilter = 'expiring';
      resetChips();
      // Attiva chip "In scadenza"
      var chips = document.querySelectorAll('.chip');
      for (var i = 0; i < chips.length; i++) {
        if (chips[i].getAttribute('data-cat') === 'expiring') {
    ...
    // Scaduti → filtra prodotti scaduti
