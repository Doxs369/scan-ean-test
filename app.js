...
function loadDemoProducts() {
  // Nessun prodotto demo
}

function loadDemoShopping() {
  // Nessun elemento demo
}
...
function openProductModal(id) {
  ...
  // Carica ricette anti-spreco
  loadRecipesForProduct();
}
...
function deleteProduct() {
  ...
  // Cancella ricette associate
  deleteProductRecipes(selectedProductId);
...
function consumeProduct() {
  ...
  // Cancella ricette associate (prodotto consumato)
  deleteProductRecipes(selectedProductId);
...
// ============================================================
// RICETTE ANTI-SPRECO (Gemini API)
// ============================================================

/**
 * Carica e mostra le ricette per il prodotto selezionato
 */
function loadRecipesForProduct() {
  ...
  var recipeBadge = document.getElementById('recipe-badge');

  // Mostra ricette solo per prodotti in scadenza (<= 5 giorni)
  if (days > 5) {
  ...
  // Controlla se ricette già salvate
  var saved = Storage.loadRecipes(product.id);
  if (saved && saved.recipes && saved.recipes.length > 0) {
    renderRecipes(saved.recipes, product);
    recipeBadge.textContent = 'Salvate';
    return;
  }

  // Mostra loading
  recipeList.innerHTML = '<div class="recipe-loading"><div class="spinner"></div>Genero ricette con AI...</div>';
  recipeBadge.textContent = 'Generazione...';

  // Chiama API Gemini
  GeminiAPI.generateRecipes(product.name, product.category, days)
    .then(function(recipes) {
      // Salva ricette
      Storage.saveRecipes(product.id, recipes);
      renderRecipes(recipes, product);
      recipeBadge.textContent = 'Generate da AI';
    })
    .catch(function(error) {
      console.error('Errore generazione ricette:', error);
      // Usa ricette di fallback
      var fallback = GeminiAPI.getFallbackRecipes(product.name);
      Storage.saveRecipes(product.id, fallback);
      renderRecipes(fallback, product);
      recipeBadge.textContent = 'Ricette base';
    });
}

/**
 * Renderizza le ricette nel modal prodotto
 */
function renderRecipes(recipes, product) {
  ...
  for (var i = 0; i < recipes.length; i++) {
    ...
    html += '<div class="recipe-card-mini" onclick="openRecipeDetail(' + i + ')">' +
      '<div class="recipe-card-mini-title">&#127860; ' + r.title + '</div>' +
      '<div class="recipe-card-mini-meta">' +
        '<span>&#9200; ' + (r.prepTime || '15 min') + '</span>' +
        '<span>&#128293; ' + (r.difficulty || 'Facile') + '</span>' +
...
/**
 * Apre il dettaglio di una ricetta
 */
function openRecipeDetail(recipeIndex) {
  if (selectedProductId === null) return;

  var saved = Storage.loadRecipes(selectedProductId);
  if (!saved || !saved.recipes || !saved.recipes[recipeIndex]) return;

  var recipe = saved.recipes[recipeIndex];
  currentRecipeProductId = selectedProductId;

  document.getElementById('recipe-detail-title').textContent = recipe.title;

  var metaHtml = '';
  if (recipe.prepTime) metaHtml += '<span>&#9200; ' + recipe.prepTime + '</span>';
  if (recipe.difficulty) metaHtml += '<span>&#128293; ' + recipe.difficulty + '</span>';
  if (recipe.servings) metaHtml += '<span>&#128101; ' + recipe.servings + ' persone</span>';
  ...
  if (recipe.tips) {
    bodyHtml += '<div class="recipe-detail-section">' +
      '<div class="recipe-detail-section-title">&#128161; Suggerimento</div>' +
      '<div class="recipe-tip"><strong>Consiglio:</strong> ' + recipe.tips + '</div>' +
      '</div>';
  }
...
/**
 * Cancella le ricette associate a un prodotto
 */
...
/**
 * Pulisce ricette orfane (prodotti eliminati)
 */
function cleanupRecipes() {
  ...
  if (removed > 0) {
    console.log('Pulite ' + removed + ' ricette orfane');
  }
}
