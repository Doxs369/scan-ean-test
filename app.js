...
// ===== VARIABILI OCR DATA SCADENZA =====
var ocrExpiryImageData = null;
var ocrDetectedDate = null;
var ocrRawText = null;
var ocrConfidence = 0;
...
function init() {
  ...
  // Inizializza Tesseract.js worker per OCR
  initTesseract();
...
/**
 * Resetta la sezione OCR allo stato iniziale
 */
function resetOcrSection() {
  var previewBox = document.getElementById('ocr-preview-box');
  ...
  var hintDiv = document.getElementById('ocr-hint');
  ...
  if (btn) {
    btn.disabled = false;
    btn.innerHTML = '&#128248; Scatta foto della data di scadenza';
  }
  if (hintDiv) hintDiv.style.display = 'block';
}

/**
 * Mostra/nasconde la sezione OCR
 */
function showOcrSection(show) {
  var section = document.getElementById('expiry-ocr-section');
  if (section) {
...
function handleCameraPhoto(event) {
  ...
  var processPhoto = function(dataUrl) {
    ...
    // Mostra sezione OCR dopo aver aggiunto la foto
    showOcrSection(true);
  };
...
// ============================================================
// OCR DATA SCADENZA - TESSERACT.JS
// ============================================================
...
function initTesseract() {
  ...
  // Il worker verrà creato on-demand per non rallentare l'avvio
  console.log('Tesseract.js pronto per OCR');
}

/**
 * Cattura un frame dalla fotocamera attiva e avvia OCR per la data di scadenza
 */
function captureExpiryDate() {
  ...
  // Cattura frame dalla fotocamera
  var frameData = Camera.captureFrame();
  if (!frameData) {
    showToast('&#10060; Impossibile catturare frame dalla fotocamera');
    return;
  }

  ocrExpiryImageData = frameData;
  ...
  statusDiv.className = 'ocr-status ocr-scanning';
  btn.disabled = true;
  btn.innerHTML = '&#9200; Elaborazione...';
  hintDiv.style.display = 'none';

  // Avvia OCR
  processOCR(frameData);
}

/**
 * Esegue OCR su un'immagine usando Tesseract.js
 */
function processOCR(imageDataUrl) {
  if (typeof Tesseract === 'undefined') {
    showToast('&#10060; OCR non disponibile. Inserisci la data manualmente.');
    resetOcrUI();
    return;
  }

  // Crea worker Tesseract con italiano e inglese
  Tesseract.recognize(
    imageDataUrl,
    'ita+eng',
    {
      logger: function(m) {
        ...
        var statusDiv = document.getElementById('ocr-status');
        if (statusDiv && m.status === 'recognizing text') {
          statusDiv.textContent = '&#128269; Riconoscimento... ' + Math.round(m.progress * 100) + '%';
        }
      }
    }
  ).then(function(result) {
    ...
    console.log('OCR Risultato:', ocrRawText, 'Confidence:', ocrConfidence);

    // Estrai data dal testo
    var detectedDate = extractDateFromText(ocrRawText);
    ocrDetectedDate = detectedDate;

    // Mostra modal di conferma
    showOcrConfirmModal(detectedDate);
    resetOcrUI();

  }).catch(function(err) {
    console.error('Errore OCR:', err);
    showToast('&#10060; Errore OCR: ' + (err.message || 'Sconosciuto'));
    resetOcrUI();
  });
}

/**
 * Estrae una data dal testo OCR.
 * Supporta formati: DD/MM/YYYY, MM/DD/YYYY, YYYY-MM-DD, DD-MM-YYYY, 
 * DD.MM.YYYY, e varianti a 2 cifre per anno
 */
function extractDateFromText(text) {
  ...
  // Pattern 1: DD/MM/YYYY o DD/MM/YY
  var pattern1 = /\b(\d{1,2})[\/\.-](\d{1,2})[\/\.-](\d{2,4})\b/g;
  var match;
  while ((match = pattern1.exec(cleanText)) !== null) {
    ...
    // Se year è a 2 cifre, assumi 20xx
  ...
  var pattern2 = /\b(\d{4})[\/\.-](\d{1,2})[\/\.-](\d{1,2})\b/g;
  while ((match = pattern2.exec(cleanText)) !== null) {
    var year2 = parseInt(match[1]);
    var month2 = parseInt(match[2]);
    var day2 = parseInt(match[3]);
    if (day2 >= 1 && day2 <= 31 && month2 >= 1 && month2 <= 12 && year2 >= 2020 && year2 <= 2050) {
      return formatDateForInput(year2, month2, day2);
    }
  }

  // Pattern 3: "DD NomeMese YYYY" o "NomeMese YYYY"
  var monthNamesRegex = new RegExp(
    '\b(\d{1,2})?\s*(' + Object.keys(monthMap).join('|') + ')\s*(\d{2,4})?\b',
    'gi'
  );
  while ((match = monthNamesRegex.exec(lowerText)) !== null) {
    var day3 = match[1] ? parseInt(match[1]) : 1;
    var monthName = match[2].toLowerCase();
    var year3 = match[3] ? parseInt(match[3]) : new Date().getFullYear();
    if (year3 < 100) year3 += 2000;
    var month3 = parseInt(monthMap[monthName]);
    if (day3 >= 1 && day3 <= 31 && month3 && year3 >= 2020 && year3 <= 2050) {
      return formatDateForInput(year3, month3, day3);
    }
  }

  // Pattern 4: "scade il ..." o "da consumare entro ..." o "best before ..."
  var contextPatterns = [
    /(?:scad|consum|entro|before|best before|use by|exp|expiration|data)[\s\w:]*?(\d{1,2})[\/\.-](\d{1,2})[\/\.-](\d{2,4})/i,
    /(?:scad|consum|entro|before|best before|use by|exp|expiration|data)[\s\w:]*?(\d{4})[\/\.-](\d{1,2})[\/\.-](\d{1,2})/i
  ];
  for (var p = 0; p < contextPatterns.length; p++) {
    var ctxMatch = cleanText.match(contextPatterns[p]);
    if (ctxMatch) {
      var d = parseInt(ctxMatch[1]);
      var m = parseInt(ctxMatch[2]);
      var y = parseInt(ctxMatch[3]);
      if (p === 1) { y = parseInt(ctxMatch[1]); m = parseInt(ctxMatch[2]); d = parseInt(ctxMatch[3]); }
      if (y < 100) y += 2000;
      if (d >= 1 && d <= 31 && m >= 1 && m <= 12 && y >= 2020 && y <= 2050) {
        return formatDateForInput(y, m, d);
      }
    }
  }

  // Pattern 5: Solo numeri attaccati (es. 31122026)
  var pattern5 = /\b(\d{2})(\d{2})(\d{4})\b/g;
  while ((match = pattern5.exec(cleanText)) !== null) {
    var day5 = parseInt(match[1]);
    var month5 = parseInt(match[2]);
    var year5 = parseInt(match[3]);
    if (day5 >= 1 && day5 <= 31 && month5 >= 1 && month5 <= 12 && year5 >= 2020 && year5 <= 2050) {
      return formatDateForInput(year5, month5, day5);
    }
  }
...
/**
 * Formatta una data come YYYY-MM-DD per input type="date"
 */
...
/**
 * Mostra il modal di conferma data OCR
 */
function showOcrConfirmModal(detectedDate) {
  var modal = document.getElementById('ocr-confirm-modal');
  var img = document.getElementById('ocr-confirm-img');
  var dateInput = document.getElementById('ocr-confirm-date');
  var textDiv = document.getElementById('ocr-detected-text');
  var confDiv = document.getElementById('ocr-confidence');

  img.src = ocrExpiryImageData;

  if (detectedDate) {
    dateInput.value = detectedDate;
    textDiv.style.display = 'block';
    textDiv.textContent = 'Testo rilevato: "' + (ocrRawText || '').substring(0, 200) + '"';

    var confClass = ocrConfidence >= 80 ? 'high' : (ocrConfidence >= 50 ? 'medium' : 'low');
    var confText = ocrConfidence >= 80 ? 'Alta' : (ocrConfidence >= 50 ? 'Media' : 'Bassa');
    confDiv.className = 'ocr-confidence ' + confClass;
    confDiv.innerHTML = '&#128269; Affidabilità OCR: <strong>' + confText + '</strong> (' + Math.round(ocrConfidence) + '%)';
  } else {
    dateInput.value = '';
    textDiv.style.display = 'block';
    textDiv.textContent = 'Testo rilevato: "' + (ocrRawText || 'Nessun testo').substring(0, 200) + '"';
    confDiv.className = 'ocr-confidence low';
    confDiv.innerHTML = '&#9888;&#65039; Nessuna data rilevata automaticamente. Inseriscila manualmente.';
  }
...
/**
 * Conferma la data OCR e la inserisce nel form
 */
function confirmOcrDate() {
  ...
  if (dateInput.value) {
    expiryInput.value = dateInput.value;
    showToast('&#9989; Data di scadenza impostata: ' + formatDate(dateInput.value));
  }
...
/**
 * Riprova la cattura OCR
 */
function retryOcrCapture() {
  document.getElementById('ocr-confirm-modal').classList.remove('show');
  setTimeout(function() {
    captureExpiryDate();
  }, 300);
}
...
/**
 * Salta OCR e inserisce manualmente
 */
function skipOcrAndManual() {
  document.getElementById('ocr-confirm-modal').classList.remove('show');
